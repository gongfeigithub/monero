...
Core tests take longer than any other BitDark tests, due to the high amount of computational work involved in validating core components.
...
To run only BitDark's core tests (after building):
...
To run only BitDark's crypto tests (after building):
...
To run only BitDark's hash tests (after building):
...
To run only BitDark's performance tests (after building):
...
To run only BitDark's unit tests (after building):
